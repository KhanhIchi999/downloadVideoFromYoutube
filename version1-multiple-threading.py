...
def download_videos(url, task_name):
    ...
    for url in video_urls:
        # Create a thread for each video download
        thread_video = threading.Thread(target=download_video, args=(url, task_name))
        threads.append(thread_video)
        thread_video.start()
    
    # Wait for all threads to complete
    for thread_video in threads:
        thread_video.join()
